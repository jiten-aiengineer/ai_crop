...
import { catalog, catalogCrops, CatalogProduct, productSupportsCrop, searchCatalog } from './lib/catalog';
...
function HomeView({ nav, t, language, location, mascotPreparing, takePhoto, uploadPhotos }: { nav: (view: View) => void; t: Copy; language: LanguageCode; location: string; mascotPreparing: boolean; takePhoto: () => void; uploadPhotos: () => void }) {
  return <><section className="hero home-hero"><div className="hero-copy home-hero-copy"><div className="endorsed"><img src="/clsl-logo.png" alt="Crop Life Science Limited" /><span>{t.developed}<br /><b>Crop Life Science Limited</b></span></div><p className="eyebrow"><span /> Crop Life AI</p><h1>{t.heroTitle}<br /><em>{t.heroAccent}</em></h1><p>{t.homeHeroBrief}</p><div className="home-hero-actions"><button type="button" onClick={() => nav('inspect')}>⌾ {t.inspectTitle}</button><button type="button" className="secondary" onClick={() => nav('assistant')}>✦ {t.askMitra}</button></div><div className="trust-row"><span>✓ {t.imageAssessment}</span><span>✓ {t.catalogProducts}</span><span>✓ CIB&RC crop map</span></div></div><MascotGuide t={t} language={language} preparing={mascotPreparing} takePhoto={takePhoto} uploadPhotos={uploadPhotos} ask={() => nav('assistant')} /></section>
...
function MascotGuide({ t, language, preparing, takePhoto, uploadPhotos, ask }: { t: Copy; language: LanguageCode; preparing: boolean; takePhoto: () => void; uploadPhotos: () => void; ask: () => void }) {
  ...
  return <section className="home-panel mascot-home-panel" aria-labelledby="mascot-greeting"><div className={`mascot-portrait tomato-mascot ${talking ? 'is-speaking' : ''}`}><img src="/crop-life-mitra-tomato-doctor.jpg" alt={`${t.mascotName}, Crop Life AI assistant`} /><span><img src="/clsl-logo.png" alt="" />Crop Life AI</span><div className="mascot-wave" aria-hidden="true">👋</div><div className={`mascot-dialogue ${talking ? 'speaking' : ''}`}><small>{t.mascotName} · {t.online}</small><p>{talkLines[line]}</p><i aria-hidden="true"><b /><b /><b /></i></div></div><div className="mascot-copy"><small>{t.care}</small><h2 id="mascot-greeting">{t.mascotGreeting}</h2><p className="mascot-prompt">{t.mascotPhotoPrompt}</p><div className="mascot-actions"><button type="button" onClick={takePhoto} disabled={preparing}><span aria-hidden="true">⌾</span>{preparing ? t.mascotPreparing : t.mascotTakePhoto}</button><button type="button" className="secondary" onClick={uploadPhotos} disabled={preparing}><span aria-hidden="true">↑</span>{t.mascotUpload}</button><button type="button" className="speak" onClick={speak}><span aria-hidden="true">{talking ? '■' : '🔊'}</span>{talking ? t.stopMitra : t.listenMitra}</button><button type="button" className="tertiary" onClick={ask}><span aria-hidden="true">✦ {t.askMitra}</span></button></div><div className="panel-stats"><span><b>5</b> {t.photosSupported}</span><span><b>73</b> CLSL {t.navProducts}</span></div></div></section>;
}
...
function Assistant({ openProduct, language, t }: { openProduct: (product: CatalogProduct) => void; language: LanguageCode; t: Copy }) {
  ...
  const companyPrompts = [t.assistantCompanyPromptOne, t.assistantCompanyPromptTwo, t.assistantCompanyPromptThree];
...
function Products({ initialQuery, onQuery, openProduct, t }: { initialQuery: string; onQuery: (query: string) => void; openProduct: (product: CatalogProduct) => void; t: Copy }) {
  ...
  const matches = useMemo(() => searchCatalog(initialQuery, 1000, category).map((match) => match.product).filter((product) => !crop || productSupportsCrop(product, crop)), [initialQuery, category, crop]);
  ...
  return <section className="marketplace"><div className="market-hero"><div><p className="eyebrow"><span /> {t.officialCatalog}</p><h1>{t.marketHeading}</h1><p>{t.marketIntro}</p></div><div className="catalog-stat"><b>{catalog.length}</b><span>{t.catalogProducts}</span><small>CIB&RC crop map</small></div></div><div className="market-toolbar"><label className="market-search"><span>⌕</span><input value={initialQuery} onChange={(event) => changeQuery(event.target.value)} placeholder={t.searchPlaceholder} />{initialQuery && <button onClick={() => changeQuery('')}>×</button>}</label><select value={category} onChange={(event) => changeCategory(event.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select><select value={crop} aria-label={t.approvedCropFilter} onChange={(event) => changeCrop(event.target.value)}><option value="">{t.approvedCropFilter}</option>{catalogCrops.map((item) => <option key={item}>{item}</option>)}</select></div><div className="category-chips">{categories.map((item) => <button key={item} className={category === item ? 'selected' : ''} onClick={() => changeCategory(item)}>{item}<span>{item === 'All products' ? catalog.length : catalog.filter((product) => product.category === item).length}</span></button>)}</div><div className="result-count"><b>{matches.length}</b> {t.found}{crop && <small> · {crop} · CIB&RC map</small>}</div>{matches.length ? <div className="catalog-grid">{matches.slice(0, visible).map((product) => <ProductCard key={product.id} product={product} open={() => openProduct(product)} t={t} />)}</div> : <div className="empty-state"><span>⌕</span><h3>{t.noCatalogMatch}</h3><p>{t.trySearch}</p><button onClick={() => { changeQuery(''); changeCategory('All products'); changeCrop(''); }}>{t.showAll}</button></div>}{visible < matches.length && <button className="load-more" onClick={() => setVisible((count) => count + 18)}>{t.loadMore} <span>{visible} / {matches.length}</span></button>}<p className="catalog-disclaimer">{t.resultDisclaimer}</p></section>;
}
