...
function HomeView({ nav, t, language, location, mascotPreparing, takePhoto, uploadPhotos }: { nav: (view: View) => void; t: Copy; language: LanguageCode; location: string; mascotPreparing: boolean; takePhoto: () => void; uploadPhotos: () => void }) {
  return <><section className="home-brief"><div className="home-brief-copy"><div className="home-brief-brand"><img src="/clsl-logo.png" alt="Crop Life Science Limited" /><span><small>{t.developed}</small><b>Crop Life Science Limited</b></span></div><p className="eyebrow"><span /> Crop Life AI</p><h1>{t.heroTitle}<em>{t.heroAccent}</em></h1><p className="home-brief-intro">{t.homeHeroBrief}</p><div className="home-brief-actions"><button type="button" onClick={() => nav('inspect')}>⌾ {t.inspectTitle}</button><button type="button" className="secondary" onClick={() => nav('assistant')}>✦ {t.askMitra}</button></div><div className="home-proof"><span>Photo assessment</span><i /> <span>CLSL catalogue</span><i /> <span>CIB&RC crop map</span></div></div><MascotGuide t={t} preparing={mascotPreparing} takePhoto={takePhoto} uploadPhotos={uploadPhotos} ask={() => nav('assistant')} /></section>
    <section className="quick-section home-quick-section"><div className="section-label"><span>{t.startHere}</span><p>Three ways Crop Life AI can help today</p></div><div className="quick-grid"><Quick featured icon="⌁" label={t.geminiPowered} title={t.inspectTitle} text={t.inspectCardText} onClick={() => nav('inspect')} /><Quick icon="✦" label="CROP LIFE MITRA" title={t.assistantTitle} text={t.assistantCardText} onClick={() => nav('assistant')} /><Quick icon="◫" label={t.officialCatalog} title={t.marketplace} text={t.marketCardText} onClick={() => nav('products')} /></div></section>
...
function InspectionForm({ inputRef, files, crops, setFiles, onAnalyse, loading, analysisProgress, error, t }: { inputRef: RefObject<HTMLInputElement | null>; files: File[]; crops: string[]; setFiles: (files: File[]) => void; onAnalyse: (form: HTMLFormElement) => Promise<void>; loading: boolean; analysisProgress: number; error: string; t: Copy }) {
  ...
  const chooseFiles = async (selected: File[]) => {
    const picked = [...files, ...selected].slice(0, 5);
    if (!picked.length) return;
    setPreparing(true);
...
function Assistant({ openProduct, language, t }: { openProduct: (product: CatalogProduct) => void; language: LanguageCode; t: Copy }) {
  const [mode, setMode] = useState<'crop' | 'company' | null>(null);
  const [input, setInput] = useState('');
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const cropPrompts = [t.assistantCropPromptOne, t.assistantCropPromptTwo, t.assistantCropPromptThree];
  const companyPrompts = [t.assistantCompanyPromptOne, t.assistantCompanyPromptTwo, t.assistantCompanyPromptThree, t.assistantSalesPrompt];
  const chooseMode = (nextMode: 'crop' | 'company') => { setMode(nextMode); setMessages([]); setInput(''); setError(''); };
  const send = async (preset?: string) => {
    const question = (preset ?? input).trim(); if (!question || loading || !mode) return;
    const next = [...messages, { role: 'user' as const, content: question }]; setMessages(next); setInput(''); setLoading(true); setError('');
    try { const response = await fetch('/api/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ question, history: messages, language, mode }) }); const data = await readApiResponse<{ answer?: string; products?: CatalogProduct[]; contacts?: SalesContact[] }>(response); if (!response.ok) throw new Error(data.error || 'Unable to answer.'); if (!data.answer?.trim()) throw new Error(t.assistantFormatError); setMessages([...next, { role: 'assistant', content: data.answer, products: data.products, contacts: data.contacts }]); }
    catch (problem) { setError(problem instanceof Error ? problem.message : 'Unable to answer.'); }
    finally { setLoading(false); }
  };
  if (!mode) return <section className="workspace"><PageTitle eyebrow={t.assistantEyebrow} title={t.assistantChooseTitle} text={t.assistantChooseText} /><div className="assistant-chooser"><div className="assistant-chooser-mascot"><img src="/crop-life-mitra-tomato-doctor.jpg" alt={`${t.mascotName}, Crop Life AI assistant`} /><span>Crop Life Mitra</span></div><div className="assistant-choice-grid"><button type="button" onClick={() => chooseMode('crop')}><span className="assistant-choice-icon">⌁</span><small>{t.assistantCropLabel}</small><b>{t.assistantCropTitle}</b><p>{t.assistantCropText}</p><i>{t.assistantStart} →</i></button><button type="button" onClick={() => chooseMode('company')}><span className="assistant-choice-icon">◌</span><small>CLSL</small><b>{t.assistantCompanyTitle}</b><p>{t.assistantCompanyText}</p><i>{t.assistantStart} →</i></button></div></div></section>;
  const isCompany = mode === 'company';
  const prompts = isCompany ? companyPrompts : cropPrompts;
  return <section className="workspace"><PageTitle eyebrow={isCompany ? 'CLSL & COMPANY SUPPORT' : t.assistantEyebrow} title={isCompany ? t.assistantCompanyTitle : t.askWords} text={isCompany ? t.assistantCompanyText : t.assistantIntro} /><div className="chat-shell"><div className="chat-head"><img className="assistant-mascot" src="/crop-life-mitra-tomato-doctor.jpg" alt="" /><div><b>{t.mascotName}</b><small>{isCompany ? t.assistantCompanyTitle : 'Gemini + CLSL RAG'}</small></div><button type="button" className="change-assistant-mode" onClick={() => { setMode(null); setMessages([]); setInput(''); setError(''); }}>{t.assistantChangeTopic}</button><span className="online">{t.connected}</span></div><div className="chat-body"><div className="ai-message">{isCompany ? t.assistantCompanyGreeting : t.assistantCropGreeting}</div><div className="suggestions">{prompts.map((prompt) => <button type="button" key={prompt} onClick={() => void send(prompt)} disabled={loading}>{prompt}</button>)}</div>{messages.map((message, index) => <div key={index} className={message.role === 'user' ? 'user-message' : 'ai-message'}><p>{message.content}</p>{message.products && message.products.length > 0 && <div className="chat-products">{message.products.map((product) => <button key={product.id} onClick={() => openProduct(product)}>{product.image && <img src={product.image} alt="" />}<span><b>{product.name}</b><small>{product.commonName}</small></span></button>)}</div>}{message.contacts && message.contacts.length > 0 && <div className="chat-contacts">{message.contacts.map((contact) => <article key={`${contact.email}-${contact.territory}`}><span className="contact-avatar">{initials(contact.name)}</span><div><small>{t.officialContact}</small><b>{contact.name}</b><p>{contact.designation} · {contact.city || contact.territory}</p></div><ContactButtons contact={contact} message={`${t.whatsappGreeting} ${contact.city || contact.territory}. ${t.whatsappHelp}`} t={t} /></article>)}</div>}</div>)}{loading && <div className="ai-message typing">{t.checking}<span>•••</span></div>}{error && <p className="chat-error">{error}</p>}</div><div className="chat-input"><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void send()} placeholder={isCompany ? t.assistantCompanyPlaceholder : t.askPlaceholder} /><button className="send" onClick={() => void send()} disabled={!input.trim() || loading}>↑</button></div></div></section>;
}
