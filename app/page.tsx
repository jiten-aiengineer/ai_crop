...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {view === 'home' && <HomeView nav={nav} t={t} language={language} location={profile.location} mascotPreparing={mascotPreparing} takePhoto={() => mascotCameraRef.current?.click()} uploadPhotos={() => mascotUploadRef.current?.click()} />}
...
function Result({ result, onClose, openProducts, openProduct, nav, t }: { result: Diagnosis; onClose: () => void; openProducts: (query: string) => void; openProduct: (product: CatalogProduct) => void; nav: (view: View) => void; t: Copy }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Crop assessment"><div className="result-modal result-wide">
    <button className="modal-close" onClick={onClose}>×</button><p className="eyebrow"><span /> {t.probableAssessment}</p><div className="mascot-result-note"><img src="/crop-life-mitra.jpg" alt="" /><div><b>{t.mascotName}</b><p>{t.mascotReady}</p></div></div>
