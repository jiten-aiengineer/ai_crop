...
import { catalog, CatalogProduct, searchCatalog } from './lib/catalog';
...
function Result({ result, onClose, openProducts, openProduct, nav }: { result: Diagnosis; onClose: () => void; openProducts: (query: string) => void; openProduct: (product: CatalogProduct) => void; nav: (view: View) => void }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Crop assessment"><div className="result-modal result-wide"><button className="modal-close" onClick={onClose}>×</button><p className="eyebrow"><span /> Probable AI assessment</p><div className="result-head"><div><small>Crop identified</small><h2>{result.crop || 'Uncertain'}</h2></div><span className="confidence"><b>{confidence}%</b> confidence</span></div><div className="issue-box"><small>{result.additional_information_required ? 'MORE INFORMATION REQUIRED' : `POSSIBLE ISSUE · ${result.issue_type || 'UNCLASSIFIED'}`}</small><h3>{result.likely_issue || 'No clear issue identified'}</h3><p>{result.summary}</p></div><div className="symptoms"><b>Visible signs</b>{result.observed_symptoms.map((symptom) => <span key={symptom}>{symptom}</span>)}</div><div className="next-action"><b>Recommended next step</b><p>{result.recommended_next_action}</p></div><section className="matched-products"><div><small>CLSL CATALOG MATCHES</small><h3>{result.recommendations?.length ? `${result.recommendations.length} possible catalog match${result.recommendations.length === 1 ? '' : 'es'}` : 'No safe catalog match yet'}</h3></div>{result.recommendations?.length > 0 ? <div className="mini-products">{result.recommendations.map((product) => <button key={product.id} onClick={() => openProduct(product)}>{product.image ? <img src={product.image} alt="" /> : <span className="mini-fallback">CLSL</span>}<span><b>{product.name}</b><small>{product.commonName}</small></span><i>View →</i></button>)}</div> : <p className="no-match">The catalog did not produce a strong deterministic match. Add clearer photos or ask the assistant before considering treatment.</p>}</section><div className="result-actions"><button onClick={() => openProducts(`${result.crop} ${result.likely_issue}`)}>Search the full catalog</button><button className="secondary" onClick={() => nav('assistant')}>Ask a follow-up</button></div><p className="demo-disclaimer">No product is invented by Gemini. Match candidates come only from the supplied Crop Life Science catalog.</p></div></div>;
}
