  ...
  return <div className="admin-content architecture-page">
    ...
    {view === 'simple' ? <div className="architecture-simple">
      ...
      <section className="architecture-simple-grid">
        <article><span>Automatic evaluation</span><h3>Qwen checks every retained case</h3><p>Each completed inspection enters the private GPU queue. Whenever the GPU is reachable, processing starts immediately—the clock does not block it.</p></article>
        <article><span>Continuous improvement</span><h3>Quality gates create training batches</h3><p>Cases with matching crop-problem evidence and sufficient confidence enter the next dataset automatically. Weak or conflicting cases remain excluded.</p></article>
      ...
      <div className="architecture-tech-grid">
        ...
        <section><header><span>AUTOMATIC EVALUATION</span><h3>Private GPU comparison</h3></header><div className="architecture-stack"><article><b>Durable PostgreSQL queue</b><small>Every retained Gemma case is queued once</small></article><Arrow /><article><b>Qwen 3.5 9B</b><small>Private Ollama GPU evaluator through the VPC</small></article><Arrow /><article><b>Gemma–Qwen quality gate</b><small>Confidence, category, issue similarity, severity and evidence checks</small></article></div></section>
