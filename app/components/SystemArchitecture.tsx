  ...
  return <div className="admin-content architecture-page">
    ...
    {view === 'simple' ? <div className="architecture-simple">
      ...
      <section className="architecture-simple-grid">
        ...
        <article><span>Continuous improvement</span><h3>Two of three must agree</h3><p>After all three results exist, a high-confidence 2-of-3 or 3-of-3 majority creates the dataset label automatically. No expert approval queue is required; a quality reviewer can remove an unsuitable case.</p></article>
      ...
      <div className="architecture-tech-grid">
        ...
        <section><header><span>CONTINUOUS MODEL DELIVERY</span><h3>Dataset → Qwen fine-tune → validate → promote</h3></header><div className="architecture-stack"><article><b>Private S3 manifest</b><small>2-of-3 labels reference retained crop images without exposing them publicly</small></article><Arrow /><article><b>Qwen adapter fine-tuning</b><small>Continues from qwen3.5:9b; it does not train a foundation model from scratch</small></article><Arrow /><article><b>Metric-gated Qwen version</b><small>Promotes the fine-tuned adapter only when validation gates pass and no production regression is detected</small></article></div></section>
      </div>
