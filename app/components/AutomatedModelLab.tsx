...
export default function AutomatedModelLab({ data, canControl, onRefresh }: Props) {
  ...
  const runtimeAge = runtime.updated_at && currentTime ? currentTime - new Date(runtime.updated_at).getTime() : Number.POSITIVE_INFINITY;
  ...
  return <div className="admin-content automated-model-lab">
    ...
    <section className="automation-policy"><b>Automatic Qwen fine-tuning policy</b><span>If the GPU is reachable, evaluation runs immediately—even outside the scheduled 8 PM start. A qualified 2-of-3 majority enters the dataset without expert approval. Training continues from qwen3.5:9b using a versioned fine-tuned adapter; it does not create a new foundation model.</span></section>
    ...
    {tab === 'training' && <>
      <section className="admin-panel training-command"><div><p className="admin-overline">Continuous Qwen fine-tuning</p><h3>{activeRun ? `${label(activeRun.status)} · ${activeRun.run_name}` : 'Pipeline watching for the next automatic batch'}</h3><p>{automation.training_connector_configured ? 'The private GPU connector fine-tunes a versioned adapter on top of the existing qwen3.5:9b base. Eligible batches start automatically without an expert-approval queue.' : 'Evaluation and dataset preparation run automatically. Adapter fine-tuning starts after the separate private GPU training connector is connected.'}</p></div>{canControl && <button className="admin-primary" disabled={Boolean(busy) || !automation.training_connector_configured} onClick={() => void execute('train_now')}>{busy === 'train_now' ? 'Starting…' : 'Start eligible Qwen fine-tune'}</button>}</section>
    ...
    {tab === 'versions' && <>
      <div className="admin-two-column"><section className="admin-panel version-summary"><p className="admin-overline">Qwen fine-tuned versions</p><h3>Base qwen3.5:9b → adapter versions</h3><div className="version-flow">{versions.length ? versions.slice().reverse().map((item: any, index: number) => <div key={item.id}><article><span>{label(item.lifecycle_stage)}</span><b>{item.model_name}</b><small>{item.version_label}</small><em>{item.metrics?.validation_accuracy != null ? `${Math.round(Number(item.metrics.validation_accuracy) * 100)}% validation` : index === 0 ? 'Existing Qwen baseline' : 'Adapter metrics pending'}</em></article>{index < versions.length - 1 && <i>→</i>}</div>) : <p>No versions recorded.</p>}</div></section><section className="admin-panel promotion-rules"><p className="admin-overline">Automatic deployment guard</p><h3>Fine-tuned adapter promotion rules</h3><dl><div><dt>Validation accuracy</dt><dd>≥ {Math.round(Number(automation.promotion_gates?.validation_accuracy || .78) * 100)}%</dd></div><div><dt>Macro F1</dt><dd>≥ {Math.round(Number(automation.promotion_gates?.macro_f1 || .72) * 100)}%</dd></div><div><dt>Issue accuracy</dt><dd>≥ {Math.round(Number(automation.promotion_gates?.issue_accuracy || .75) * 100)}%</dd></div><div><dt>Regression</dt><dd>Must not be worse than production</dd></div></dl></section></div>
