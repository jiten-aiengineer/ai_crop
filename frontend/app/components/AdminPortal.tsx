...
function Campaigns({ items, totalSpend, onRefresh }: { items: any[]; totalSpend: number; onRefresh: () => void }) {
  ...
  const empty = { name:'', campaignType:'welcome', description:'', discountType:'fixed_amount', discountValue:'50', audience:'new_farmer', states:'', districts:'', villages:'', products:'', startDate:today, endDate:later, couponLimit:'1000', budget:'50000', randomPercentage:'10', expiryDays:'30', publish:true };
  ...
  async function create(event:FormEvent){event.preventDefault();setBusy(true);setMessage('');setProblem('');try{
    await api('campaigns',{method:'POST',body:JSON.stringify({name:draft.name,description:draft.description,campaign_type:draft.campaignType,discount_type:draft.discountType,discount_value:Number(draft.discountValue),audience:draft.audience,states:list(draft.states),districts:list(draft.districts),villages:list(draft.villages),products:list(draft.products),start_date:new Date(`${draft.startDate}T00:00:00`).toISOString(),end_date:draft.endDate?new Date(`${draft.endDate}T23:59:59`).toISOString():null,coupon_limit:Number(draft.couponLimit||0),budget:Number(draft.budget||0),random_percentage:Number(draft.randomPercentage||10),expiry_days:Number(draft.expiryDays||30),status:draft.publish?'active':'paused'})});
  ...
  return <div className="admin-content campaign-studio">
    ...
    {mode==='create'?<form className="campaign-builder" onSubmit={create}>
      ...
      <div className="campaign-form-grid">
        ...
        <section className="campaign-form-card"><header><span>2</span><div><h3>Farmer benefit</h3><p>Define the coupon value and eligible products.</p></div></header><div className="campaign-inline"><label>Benefit type<select value={draft.discountType} onChange={e=>set('discountType',e.target.value)}><option value="fixed_amount">Fixed discount (₹)</option><option value="percentage">Percentage discount (%)</option></select></label><label>Value<input type="number" min="1" step="0.5" value={draft.discountValue} onChange={e=>set('discountValue',e.target.value)} required/></label></div><label>Eligible CLSL products<input value={draft.products} onChange={e=>set('products',e.target.value)} placeholder="All products, or comma-separated product names"/><small>Leave blank when the offer applies to every active CLSL product.</small></label><div className="campaign-inline"><label>Coupon limit<input type="number" min="0" value={draft.couponLimit} onChange={e=>set('couponLimit',e.target.value)}/></label><label>Campaign budget (₹)<input type="number" min="0" value={draft.budget} onChange={e=>set('budget',e.target.value)}/></label></div></section>
