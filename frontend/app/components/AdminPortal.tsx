...
function Campaigns({ items, products: suppliedProducts = [], totalSpend, onRefresh }: { items: any[]; products?: Product[]; totalSpend: number; onRefresh: () => void }) {
  const today = new Date().toISOString().slice(0,10);
  const later = new Date(Date.now()+30*86400000).toISOString().slice(0,10);
  const empty = { name:'', campaignType:'welcome', description:'', discountType:'fixed_amount', discountValue:'50', audience:'new_farmer', states:'', districts:'', villages:'', productId:'', packing:'', startDate:today, endDate:later, couponLimit:'1000', budget:'50000', randomPercentage:'10', expiryDays:'30', publish:true };
  const [draft,setDraft]=useState(empty); const [mode,setMode]=useState<'create'|'manage'>('create');
  const [catalogueProducts,setCatalogueProducts]=useState<Product[]>(suppliedProducts);
  const [busy,setBusy]=useState(false); const [message,setMessage]=useState(''); const [problem,setProblem]=useState('');
  const [inventory,setInventory]=useState<Record<string,string>>({});
  const list=(value:string)=>value.split(',').map(item=>item.trim()).filter(Boolean);
  const totalCoupons=items.reduce((sum,item)=>sum+Number(item.total_issued||0),0);
  const redeemed=items.reduce((sum,item)=>sum+Number(item.total_redeemed||0),0);
  const assigned=items.reduce((sum,item)=>sum+Number(item.total_assigned||0),0);
  const active=items.filter(item=>item.status==='active').length;
  const set=(field:string,value:string|boolean)=>setDraft(current=>({...current,[field]:value}));
  useEffect(()=>{if(suppliedProducts.length){setCatalogueProducts(suppliedProducts);return;}void api<{items:Product[]}>('catalogue/products').then(result=>setCatalogueProducts(result.items||[])).catch(()=>{});},[suppliedProducts]);
  const products=catalogueProducts.filter(product=>product.status==='active'&&product.approval_status==='approved');
  const selectedProduct=products.find(product=>product.id===draft.productId);
  const packingOptions=(selectedProduct?.packing||'').split(/[,;/|]/).map(value=>value.trim()).filter(Boolean);
  const applyPreset=(kind:string)=>{
    const presets:Record<string,Partial<typeof empty>>={
      welcome:{name:'Welcome Offer',campaignType:'welcome',audience:'new_farmer',discountType:'fixed_amount',discountValue:'50',description:'A one-time reward automatically added after a farmer completes registration.'},
      festival:{name:'Festival Farmer Offer',campaignType:'festival',audience:'all_farmers',discountType:'percentage',discountValue:'10',description:'A time-limited seasonal reward for eligible farmers.'},
      regional:{name:'Regional Crop Offer',campaignType:'regional',audience:'targeted',discountType:'fixed_amount',discountValue:'100',description:'A location and product-specific farmer promotion.'},
      random:{name:'Farmer Surprise Reward',campaignType:'random',audience:'random',discountType:'fixed_amount',discountValue:'75',description:'A controlled random reward issued to a fixed percentage of eligible farmers.'},
    }; setDraft(current=>({...current,...presets[kind]}));
  };
  async function create(event:FormEvent){event.preventDefault();setBusy(true);setMessage('');setProblem('');try{
    const product=products.find(item=>item.id===draft.productId);
    await api('campaigns',{method:'POST',body:JSON.stringify({name:draft.name,description:draft.description,campaign_type:draft.campaignType,discount_type:draft.discountType,discount_value:Number(draft.discountValue),audience:draft.audience,states:list(draft.states),districts:list(draft.districts),villages:list(draft.villages),products:product?[product.name]:[],product_ids:product?[product.id]:[],packings:draft.packing?[draft.packing]:[],start_date:new Date(`${draft.startDate}T00:00:00`).toISOString(),end_date:draft.endDate?new Date(`${draft.endDate}T23:59:59`).toISOString():null,coupon_limit:Number(draft.couponLimit||0),budget:Number(draft.budget||0),random_percentage:Number(draft.randomPercentage||10),expiry_days:Number(draft.expiryDays||30),status:draft.publish?'active':'paused'})});
    setMessage(draft.publish?'Campaign published. Eligible farmers will receive it in their CLSL AI coupon wallet.':'Campaign saved as paused. Activate it when ready.');setDraft(empty);onRefresh();setMode('manage');
  }catch(error){setProblem(error instanceof Error?error.message:'Campaign could not be created.')}finally{setBusy(false)}}
  async function status(id:string,current:string){setBusy(true);setProblem('');try{await api(`campaigns/${id}/status`,{method:'PUT',body:JSON.stringify({status:current==='active'?'paused':'active'})});onRefresh()}catch(error){setProblem(error instanceof Error?error.message:'Status could not be updated.')}finally{setBusy(false)}}
  async function generate(id:string){const count=Number(inventory[id]||100);setBusy(true);setProblem('');try{const result=await api<{generated_count:number}>(`campaigns/${id}/bulk-generate`,{method:'POST',body:JSON.stringify({count})});setMessage(`${result.generated_count} secure seven-character coupons added to inventory.`);onRefresh()}catch(error){setProblem(error instanceof Error?error.message:'Coupon inventory could not be generated.')}finally{setBusy(false)}}
  return <div className="admin-content campaign-studio">
    <header className="campaign-hero"><div><p className="admin-overline">Farmer rewards control centre</p><h2>Campaign & coupon studio</h2><p>Create, target, publish and measure every CLSL farmer promotion from one place.</p></div><div className="campaign-mode"><button className={mode==='create'?'active':''} onClick={()=>setMode('create')}>＋ Create campaign</button><button className={mode==='manage'?'active':''} onClick={()=>setMode('manage')}>Manage & analyse</button></div></header>
    {(message||problem)&&<p className={`campaign-feedback ${problem?'error':''}`}>{problem||message}</p>}
    <section className="campaign-kpis"><Metric label="Live campaigns" value={active} note={`${items.length} campaigns configured`} tone="green"/><Metric label="Coupon inventory" value={totalCoupons} note={`${assigned} assigned to farmers`}/><Metric label="Coupons redeemed" value={redeemed} note={totalCoupons?`${Math.round(redeemed/totalCoupons*100)}% redemption rate`:'No redemptions yet'} tone="amber"/><Metric label="Dealer credit generated" value={`₹${Number(totalSpend||0).toLocaleString('en-IN')}`} note="Recorded redemption value"/></section>
    {mode==='create'?<form className="campaign-builder" onSubmit={create}>
      <section className="campaign-presets"><div><p className="admin-overline">Start with a campaign type</p><h3>Choose a proven offer structure</h3></div><div>{[['welcome','Welcome offer','Automatic after farmer registration'],['festival','Festival offer','Seasonal and time limited'],['regional','Region-wise offer','State, district or village targeting'],['random','Random reward','Controlled percentage rollout']].map(([id,title,note])=><button type="button" key={id} className={draft.campaignType===id?'selected':''} onClick={()=>applyPreset(id)}><b>{title}</b><small>{note}</small></button>)}</div></section>
      <div className="campaign-form-grid">
        <section className="campaign-form-card"><header><span>1</span><div><h3>Campaign identity</h3><p>What farmers and CLSL teams will recognise.</p></div></header><label>Campaign name<input value={draft.name} onChange={e=>set('name',e.target.value)} placeholder="Example: Kharif Welcome Reward" required/></label><label>Description<textarea rows={3} value={draft.description} onChange={e=>set('description',e.target.value)} placeholder="Explain the purpose and farmer benefit."/></label><label>Campaign category<select value={draft.campaignType} onChange={e=>set('campaignType',e.target.value)}><option value="welcome">Welcome / registration</option><option value="festival">Festival / seasonal</option><option value="regional">Regional promotion</option><option value="random">Random farmer reward</option><option value="general">General promotion</option></select></label></section>
        <section className="campaign-form-card"><header><span>2</span><div><h3>Farmer benefit</h3><p>Fixed ₹ discounts are the default; percentage offers remain available when required.</p></div></header><div className="campaign-inline"><label>Benefit type<select value={draft.discountType} onChange={e=>set('discountType',e.target.value)}><option value="fixed_amount">Fixed discount (₹) — recommended</option><option value="percentage">Percentage discount (%)</option></select></label><label>{draft.discountType==='fixed_amount'?'Discount amount (₹)':'Discount percentage (%)'}<input type="number" min="1" max={draft.discountType==='percentage'?100:undefined} step="0.5" value={draft.discountValue} onChange={e=>set('discountValue',e.target.value)} required/></label></div><label>Eligible CLSL product<select value={draft.productId} onChange={e=>{set('productId',e.target.value);set('packing','')}}><option value="">All active, approved CLSL products</option>{products.map(product=><option key={product.id} value={product.id}>{product.name} · {product.formulation||product.category}</option>)}</select><small>The farmer wallet and dealer statement will show this verified catalogue scope.</small></label><label>Eligible packing<select value={draft.packing} onChange={e=>set('packing',e.target.value)} disabled={!selectedProduct}><option value="">All eligible packs</option>{packingOptions.map(pack=><option key={pack}>{pack}</option>)}</select><small>{selectedProduct&&!packingOptions.length?'No structured packing is recorded for this product; the offer applies to all packs.':'Choose a pack only when the discount is pack-specific.'}</small></label><div className="campaign-inline"><label>Coupon limit<input type="number" min="0" value={draft.couponLimit} onChange={e=>set('couponLimit',e.target.value)}/></label><label>Campaign budget (₹)<input type="number" min="0" value={draft.budget} onChange={e=>set('budget',e.target.value)}/></label></div></section>
