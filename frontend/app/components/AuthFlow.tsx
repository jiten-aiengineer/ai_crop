...
type ReverseGeocode = {
  ...
  localityInfo?: { administrative?: Array<{ name?: string; description?: string; adminLevel?: number }> };
};

function districtFromLocation(data: ReverseGeocode) {
  const authorities = data.localityInfo?.administrative || [];
  return authorities.find(item => /district/i.test(item.description || ''))?.name
    || authorities.find(item => item.adminLevel === 5)?.name
    || data.city
    || data.locality
    || '';
}

export const authCopy: Record<LanguageCode, Record<string, string>> = {
...
const districtLocationError: Record<LanguageCode, string> = {
  en: 'District could not be detected. Enter your district manually.',
  hi: 'जिला नहीं मिल सका। अपना जिला स्वयं दर्ज करें।',
  gu: 'જિલ્લો મળી શક્યો નથી. તમારો જિલ્લો જાતે દાખલ કરો.',
  mr: 'जिल्हा सापडला नाही. तुमचा जिल्हा स्वतः भरा.',
  bn: 'জেলা শনাক্ত করা যায়নি। আপনার জেলা লিখুন।',
  bho: 'जिला ना मिलल। अपना जिला खुद लिखीं।',
};
...
export function AuthFlow({ onComplete }: { onComplete: (token: string, user: PublicUser) => void }) {
  ...
  const qrInput = useRef<HTMLInputElement>(null); const c: Record<string, string> = { ...authCopy[language], locateError: districtLocationError[language] };
  ...
  const locate = () => {
    ...
    navigator.geolocation.getCurrentPosition(async position => {
      ...
      try {
        ...
        endpoint.searchParams.set('latitude', String(point.latitude));
        endpoint.searchParams.set('longitude', String(point.longitude));
        endpoint.searchParams.set('localityLanguage', 'en');
        const response = await fetch(endpoint);
        const data = await response.json() as ReverseGeocode;
        const properDistrict = response.ok ? districtFromLocation(data) : '';
        if (!properDistrict) throw new Error(c.locateError);
        setDistrict(properDistrict);
        setState(data.principalSubdivision || '');
      } catch { setCoordinates(null);setError(c.locateError); }
      setLoading(false);
  ...
  const startOtp = async (event: FormEvent) => { event.preventDefault();setError('');if(!district.trim()&&role!=='dealer')return setError(c.required);if(role==='dealer'&&(!dealer||!dealerConfirmed))return setError(c.dealerRequired);setLoading(true);try{await api('send-otp',{mobile_number:`+91${normalizedMobile}`});setOtp('');setStep('otp');}catch(problem){fail(problem);}finally{setLoading(false);} };
  const finish = async (event: FormEvent) => { event.preventDefault();setLoading(true);setError('');try{const verified=await api<{session_token:string;user:PublicUser}>('verify-otp',{mobile_number:`+91${normalizedMobile}`,otp});const session=verified.session_token;setToken(session);if(!verified.user.requires_onboarding){localStorage.setItem('clsl_auth_token',session);onComplete(session,verified.user);return;}const data=await api<{user:PublicUser}>('profile',{role,first_name:firstName,preferred_language:language,email:email||null,city:null,district:district||null,village:null,state:state||null,social_media_used:social,acquisition_source:source||null,referral_code:role==='farmer'&&referral?referral:null,dealer_code:role==='dealer'?dealerCode:null,location_latitude:coordinates?.latitude,location_longitude:coordinates?.longitude,location_consent:Boolean(coordinates)},session);localStorage.setItem('clsl_auth_token',session);onComplete(session,data.user);}catch(problem){fail(problem);}finally{setLoading(false);} };

  const steps: Step[] = ['language','basics','role','details','otp'];
  const stepNumber = steps.indexOf(step) + 1;

  return <main className="auth-shell"><section className="auth-visual"><div className="auth-brand"><img src="/clsl-logo.png" alt="Crop Life Science Limited"/><span><b>CLSL AI</b><small>Crop care, made smarter.</small></span></div><div className="auth-welcome"><small>CROP LIFE SCIENCE LIMITED</small><h1>{c.welcome}</h1><p>{c.basicsHelp}</p><div className="auth-trust"><span>✦ AI crop care</span><span>✓ Trusted CLSL support</span></div></div><figure className="auth-mascot"><img src="/crop-life-mitra-tomato-doctor.jpg" alt="Crop Life Mitra, the CLSL AI crop doctor"/><figcaption><b>Crop Life Mitra</b><small>Your smart crop companion</small></figcaption></figure><div className="auth-field-art"><SprayerScene /></div></section><section className="auth-panel"><header className="auth-panel-head"><span>STEP {String(stepNumber).padStart(2,'0')} OF 05</span><b>{Math.round(stepNumber / steps.length * 100)}% complete</b></header><div className="auth-progress">{steps.map((item,index)=><i key={item} className={index<stepNumber?'active':''}/>)}</div>{step!=='language'&&<button type="button" className="auth-back" onClick={()=>{setError('');setStep(previous[step]);}}>← {c.back}</button>}{error&&<p className="auth-error" role="alert">{error}</p>}
    ...
    {step==='details'&&<form className="auth-step auth-details" onSubmit={startOtp}><h2>{c.profile}</h2>{role!=='dealer'&&<><section className="district-location-card"><label>{c.district}<input value={district} onChange={e=>{setDistrict(e.target.value);setCoordinates(null);}} required autoComplete="address-level2" placeholder={c.district}/></label><button type="button" className={`location-consent ${coordinates?'ready':''}`} onClick={locate} disabled={loading}>⌖ {loading?c.loading:coordinates?`${c.locationReady}: ${district}`:c.useLocation}</button></section><label>{c.email}<input type="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><fieldset><legend>{c.social}</legend><div className="social-grid">{socialOptions.map(item=><label key={item}><input type="checkbox" checked={social.includes(item)} onChange={()=>setSocial(current=>current.includes(item)?current.filter(value=>value!==item):[...current,item])}/><span>{item}</span></label>)}</div></fieldset><label>{c.source}<select value={source} onChange={e=>setSource(e.target.value)}><option value="">—</option>{sourceOptions.map(item=><option key={item}>{item}</option>)}</select></label></>}
    ...
    {token&&<span hidden>{token}</span>}</section></main>;
}
